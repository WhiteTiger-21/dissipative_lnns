...
def get_args():
    ...
    parser.add_argument('--name', default='dp', type=str, help='only one option right now')
...
def train(args):
    ...
    if args.baseline :
        ...
        model = MLP(args.input_dim, args.hidden_dim, 2, args.nonlinearity)
        model = DLNN(model=model)
        ...
        if args.baseline :
            ...
            for step in range(steps+1):
                ...
                for b,(xb ,dxdtb) in enumerate(train_loader):
                    ...
                    loss = loss_fn(dxdtb[:,2:], dxdt_hat[:,2:])
                    loss.backward() ; optim.step() ; optim.zero_grad()
                    stats['train_loss'].append(loss.item())
